	...
	public void updateCreatures() {
		...
		for(Creature c :  _creatures) {
				...
				if(c instanceof Aggressor) {
					...
					//if the target is eaten and the create implements the Spawner interface (wolf)
					if(target != null && target.isAlive() == false && (c instanceof Spawner)){
						Creature newCreature = ((Spawner) c).spawnNewCreature();

						if(newBabies == null)
						{
							newBabies = new ArrayList<Creature>();
						}
						newBabies.add(newCreature);
					}
				}
			}

		if(newBabies != null) {
			_creatures.addAll(newBabies);
		}
	}
}